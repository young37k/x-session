          ...
          {mode === "register" && (
            <>
              ...
              <select
                ...
                onChange={(e) => setForm({ ...form, regionCity: e.target.value })}
...
function ProfilePanel({ user, onUpdate, saving }) {
  ...
  useEffect(() => setForm({ ...user, division: user?.division || "전체학년", regionCity: user?.regionCity || "", groupName: user?.groupName || "" }), [user]);
      ...
      <Card className="rounded-[28px] border-0 bg-white shadow-xl">
        ...
        <CardContent>
          <form className="grid gap-4" onSubmit={submit}>
            ...
            <div className="grid gap-2">
              <Label>이름</Label>
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="정확한 기록 관리를 위해 본명 사용 권장" />
            ...
            <div className="grid gap-2">
              <Label>소속</Label>
              <Input value={form.groupName || ""} onChange={(e) => setForm({ ...form, groupName: e.target.value })} placeholder="예: 서울체고, OO클럽, OO실업팀" />
            </div>
            ...
            <Button type="submit" disabled={saving} className="rounded-2xl bg-blue-900 hover:bg-blue-800">
              {saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}프로필 저장
            </Button>

            {saved && <div className="rounded-2xl bg-emerald-50 px-4 py-3 text-sm text-emerald-700">프로필을 갱신했다. X-Session 기본값에도 반영된다.</div>}
          </form>
        </CardContent>
      </Card>
...
function XSessionApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(appServices.auth, async (user) => {
      ...
      try {
        const profileRef = doc(appServices.db, "users", user.uid);
        const profileSnap = await getDoc(profileRef);

        let nextProfile;

        if (profileSnap.exists()) {
          nextProfile = fromFirestoreProfile(user.uid, profileSnap.data());
        } else {
          const pending = pendingProfileRef.current;
          nextProfile = {
            id: user.uid,
            uid: user.uid,
            name: pending?.name || user.displayName || user.email?.split("@")[0] || "사용자",
            email: pending?.email || user.email || "",
            club: "",
            clubName: "",
            groupName: pending?.groupName || "",
            regionCity: pending?.regionCity || "",
            division: pending?.division || "전체학년",
            avatar: "",
            photoURL: "",
            photoPath: "",
          };
        ...
        setEditingSessionId(null);
        setUi((prev) => ({ ...prev, activeTab: adminRequested && isAdminEmail(nextProfile.email) ? "admin" : "dashboard" }));

        await loadUsersAndSessions(appServices.db);
      } catch (error) {
        setGlobalError(error.message || "프로필 로딩에 실패했다.");
      } finally {
        setAuthLoading(false);
      }
    });
    ...
    return () => unsubscribe();
  }, [appServices, loadUsersAndSessions]);
  ...
  async function handleUpdateProfile(nextUser) {
    ...
    try {
      ...
      setGlobalError(error.message || "프로필 저장에 실패했다.");
    } finally {
