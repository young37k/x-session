...
function RankingBoard({ users, sessions, currentUserId, currentUser }) {
  ...
  const rankingUsers = useMemo(() => {
    return users
      .filter((user) => (user.ranking?.totalSessions || 0) > 0)
      .filter((user) => rankingFilters.regionCity === "all" ? true : user.regionCity === rankingFilters.regionCity)
      .map((user) => ({
        userId: user.id,
        name: user.name,
        groupName: user.groupName || "",
        ...
        totalScore: user.ranking?.totalScore || 0,
        avgArrow: user.ranking?.avgArrowScore || 0,
        bestSession: user.ranking?.bestSessionScore || 0,
        sessions: user.ranking?.totalSessions || 0,
        xCount: user.ranking?.totalXCount || 0,
        latestDate: user.ranking?.lastSessionAt || "",
      }))
      .sort((a, b) => {
