...
function getRankingGroup(division, gender) {
  ...
  if (/초등/.test(compact) && /(5~6학년|5-6학년|5～6학년|고학년)/.test(compact)) return "초등부(고학년)";
...
const ENABLE_OFFICIAL_RECORDS = false;

// 공식 결과 목록은 현재 초기화 상태다.
// 새 공식기록 자료를 다시 정리하기 전까지 앱 내장 공식/샘플 기록은 표시하지 않는다.
...
function RankingBoard({ users, sessions, currentUser, currentUserId, officialClaims = [], onRequestOfficialClaim, appServices = null }) {
  ...
  useEffect(() => {
    if (!ENABLE_OFFICIAL_RECORDS || hideOfficialRecords || !appServices?.db) {
      setRemoteRankingEntries([]);
      setRemoteRankingLoading(false);
      setRemoteRankingNotice(ENABLE_OFFICIAL_RECORDS ? "" : "공식/샘플 기록은 초기화 상태입니다. 현재 랭킹은 개인 기록만 표시합니다.");
      return;
    }
    let cancelled = false;
  ...
  const runRankingSearch = useCallback(() => {
    ...
    setRemoteRankingNotice(ENABLE_OFFICIAL_RECORDS ? "검색 조건 기준으로 Firestore 공식기록을 불러오는 중입니다." : "공식/샘플 기록은 초기화 상태입니다. 현재 랭킹은 개인 기록만 표시합니다.");
  }, [rankingFilters]);
